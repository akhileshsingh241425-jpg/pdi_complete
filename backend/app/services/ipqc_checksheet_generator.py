...
def generate_ipqc_checksheet(
    ...
    ws.merge_cells('N2:O2'); _cell(ws, 2, 14, '01/12/2024', FONT_BOLD, alignment=ALIGN_CENTER_NW)
    ...
    ws.merge_cells('H5:N6'); _cell(ws, 5, 8, 'Monitoring Result', FONT_BOLD, HEADER_FILL, ALIGN_CENTER_NW)
...
def _write_all_stages(ws, prefix, start, cell_mfr, cell_eff, cable_len, golden, shift, date):
    ...
    _cell(ws, 9, 4, 'once', B, alignment=AC)
    _cell(ws, 9, 5, 'per shift', B, alignment=AC)
    ws.merge_cells('F9:G9')
    _cell(ws, 9, 6, 'As Per PO', B)
    ...
    _cell(ws, 10, 4, 'once', B, alignment=AC)
    _cell(ws, 10, 5, 'per shift', B, alignment=AC)
    ...
    _cell(ws, 11, 4, 'once', B, alignment=AC)
    _cell(ws, 11, 5, 'per shift', B, alignment=AC)
    ...
    _cell(ws, 13, 4, 'once', B, alignment=AC)
    _cell(ws, 13, 5, 'per shift', B, alignment=AC)
    ...
    _cell(ws, 14, 4, 'Once', B, alignment=AC)
    _cell(ws, 14, 5, 'per shift', B, alignment=AC)
    ...
    _cell(ws, 15, 4, 'once', B, alignment=AC); _cell(ws, 15, 5, 'per shift', B, alignment=AC)
    ...
    _cell(ws, 16, 4, 'once', B, alignment=AC); _cell(ws, 16, 5, 'per shift', B, alignment=AC)
    ...
    _cell(ws, 17, 4, 'once', B, alignment=AC); _cell(ws, 17, 5, 'per shift', B, alignment=AC)
    ...
    _cell(ws, 18, 4, 'once', B, alignment=AC); _cell(ws, 18, 5, 'per shift', B, alignment=AC)
    ...
    _cell(ws, 19, 4, 'once', B, alignment=AC); _cell(ws, 19, 5, 'per shift', B, alignment=AC)
    ws.merge_cells('F19:G19'); _cell(ws, 19, 6, 'ATW Stringer Specification', F14, alignment=AV)
    ws.merge_cells('H19:N19'); _cell(ws, 19, 8, 'Monitoring of ATW STRINGER — Verified', F14, alignment=AVNW)
    _cell(ws, 19, 15, 'OK', N, alignment=AC)

    _cell(ws, 20, 3, 'Cell Cross cutting', B, alignment=AV)
    _cell(ws, 20, 4, 'once', B, alignment=AC); _cell(ws, 20, 5, 'per shift', B, alignment=AC)
    ...
    ws.merge_cells('E30:E31'); _cell(ws, 30, 5, 'per shift', B, alignment=AC)
    ...
    _cell(ws, 30, 8, f'Ribbon to cell soldering peel strength: {", ".join(f"{v:.2f}N" for v in peel_vals)}', F14, alignment=AC)
    ...
    _cell(ws, 34, 8, f'Bottom: {bot_dist}mm', B, alignment=AM)
    ...
    ws.merge_cells('F40:G40'); _cell(ws, 40, 6, 'Creepage distance: 12±0.15mm', B, alignment=AV)
    # 3 readings — each column has T and B together: "T-12.04mm/B-12.09mm"
    t1 = round(random.uniform(11.85, 12.15), 2); b1 = round(random.uniform(11.85, 12.15), 2)
    t2 = round(random.uniform(11.85, 12.15), 2); b2 = round(random.uniform(11.85, 12.15), 2)
    t3 = round(random.uniform(11.85, 12.15), 2); b3 = round(random.uniform(11.85, 12.15), 2)
    ws.merge_cells('H40:I40')
    _cell(ws, 40, 8,  f'T-{t1}mm/B-{b1}mm', N, alignment=AC)
    ws.merge_cells('J40:K40')
    _cell(ws, 40, 10, f'T-{t2}mm/B-{b2}mm', N, alignment=AC)
    ws.merge_cells('L40:N40')
    _cell(ws, 40, 12, f'T-{t3}mm/B-{b3}mm', N, alignment=AC)
    ...
    _cell(ws, 43, 4, 'Every 4h', B, alignment=AC); _cell(ws, 43, 5, 'per shift', B, alignment=AC)
    ...
    _cell(ws, 44, 4, 'once', B, alignment=AC); _cell(ws, 44, 5, 'per shift', B, alignment=AC)
    ...
    _cell(ws, 46, 4, 'once', B, alignment=AC); _cell(ws, 46, 5, 'per shift', B, alignment=AC)
    ...
    ws.merge_cells('D47:D48'); _cell(ws, 47, 4, 'once', B, alignment=AC)
    ws.merge_cells('E47:E48'); _cell(ws, 47, 5, 'Per shift', B, alignment=AC)
    ...
    ws.merge_cells('E49:E50'); _cell(ws, 49, 5, 'Per shift', B, alignment=AC)
    ...
    ws.merge_cells('D53:D57'); _cell(ws, 53, 4, '5 pieces', B, alignment=AC)
    ws.merge_cells('E53:E57'); _cell(ws, 53, 5, 'per shift', B, alignment=AC)
    ...
    _cell(ws, 56, 6, 'Pre EL Visual Criteria', F14, alignment=AV)
    ...
    _cell(ws, 58, 1, 13, B, alignment=ACNW)
    ...
    _cell(ws, 58, 4, 'once', B, alignment=AC); _cell(ws, 58, 5, 'per shift', B, alignment=AC)
    ...
    _cell(ws, 59, 4, 'once', B, alignment=AC); _cell(ws, 59, 5, 'per shift', B, alignment=AC)
    ws.merge_cells('F59:G59'); _cell(ws, 59, 6, '400±30°C', B, alignment=AV)
    iron_t1 = _rr(400, 20)
    ws.merge_cells('H59:N59'); _cell(ws, 59, 8, f'Time: {_shift_start(shift)}  {iron_t1}°C', B, alignment=ALT)
    ...
    _cell(ws, 60, 4, 'once', B, alignment=AC); _cell(ws, 60, 5, 'per shift', B, alignment=AC)
    ...
    _cell(ws, 61, 4, 'once', B, alignment=AC); _cell(ws, 61, 5, 'per shift', B, alignment=AC)
    ...
    _cell(ws, 62, 4, 'once', B, alignment=AC); _cell(ws, 62, 5, 'per shift', B, alignment=AC)
    ws.merge_cells('F62:G62'); _cell(ws, 62, 6, '400±30°C', B, alignment=AV)
    iron_t2 = _rr(400, 20)
    ws.merge_cells('H62:N62'); _cell(ws, 62, 8, f'Time: {_shift_start(shift)}  {iron_t2}°C', B, alignment=AT)
    ...
    _cell(ws, 63, 4, 'once', B, alignment=AC); _cell(ws, 63, 5, 'per shift', B, alignment=AC)
    ws.merge_cells('F63:G63'); _cell(ws, 63, 6, 'Process Parameter of jinchen Laminator', F14, alignment=AV)
    ...
    _cell(ws, 64, 4, 'once', B, alignment=AC); _cell(ws, 64, 5, '24h', B, alignment=AC)
    ...
    ws.merge_cells('H65:N65'); _cell(ws, 65, 8, 'Refer Document GSPL/IPQC/QC/001', F14, alignment=AV)
    ...
    ws.merge_cells('H66:N66'); _cell(ws, 66, 8, 'Refer Document GSPL/IPQC/QC/001', F14, alignment=AV)
    ...
    _cell(ws, 67, 4, '5 pieces', B, alignment=AC); _cell(ws, 67, 5, 'per shift', B, alignment=AC)
    ...
    ws.merge_cells('D68:D72'); _cell(ws, 68, 4, '5 pieces', B, alignment=AC)
    ws.merge_cells('E68:E72'); _cell(ws, 68, 5, 'per shift', B, alignment=AC)
    ...
    for idx, r in enumerate([68, 69, 70, 71, 72]):
        ws.merge_cells(f'H{r}:I{r}'); _cell(ws, r, 8, 'S.No.', F14, alignment=AC)
        ws.merge_cells(f'J{r}:N{r}')
    ...
    ws.merge_cells('D74:D78'); _cell(ws, 74, 4, '5 pieces', B, alignment=AC)
    ws.merge_cells('E74:E78'); _cell(ws, 74, 5, 'per shift', B, alignment=AC)
    ws.merge_cells('F74:G78'); _cell(ws, 74, 6, 'Post Lam Visual Inspection Criteria', F14, alignment=AV)

    snos_vis = _gen_serials(prefix, start, 5)
    for idx, r in enumerate([74, 75, 76, 77, 78]):
        ws.merge_cells(f'H{r}:I{r}'); _cell(ws, r, 8, 'S.No', B, alignment=AM)
        ws.merge_cells(f'J{r}:N{r}')
    ...
    _cell(ws, 79, 4, '1 set', B, alignment=AC); _cell(ws, 79, 5, 'per shift', B, alignment=AC)
    ...
    ws.merge_cells('H80:N81'); _cell(ws, 80, 8, 'Refer Document GSPL/IPQC/QC/011', F14, alignment=AC)
    ...
    ws.merge_cells('D83:D84'); _cell(ws, 83, 4, 'once', B, alignment=AC)
    ws.merge_cells('E83:E84'); _cell(ws, 83, 5, 'Per shift', B, alignment=AC)
    ...
    _cell(ws, 85, 4, 'once', B, alignment=AC); _cell(ws, 85, 5, 'Per shift', B, alignment=AC)
    ...
    _cell(ws, 86, 4, 'once', B, alignment=AC); _cell(ws, 86, 5, 'Per shift', B, alignment=AC)
    ...
    _cell(ws, 88, 4, 'once', B, alignment=AC); _cell(ws, 88, 5, 'per shift', B, alignment=AC)
    ...
    _cell(ws, 89, 4, 'once', B, alignment=AC); _cell(ws, 89, 5, 'Per shift', B, alignment=AC)
    ws.merge_cells('F89:G89'); _cell(ws, 89, 6, 'As per Specification', B, alignment=AV)
    ws.merge_cells('H89:N89'); _cell(ws, 89, 8, 'Refer Document GSPL/IPQC/QC/011', F14, alignment=AM)
    _cell(ws, 89, 15, 'OK', N, alignment=AC)

    _cell(ws, 90, 3, 'Potting material weight', B, alignment=AV)
    _cell(ws, 90, 4, 'once', B, alignment=AC); _cell(ws, 90, 5, 'Per shift', B, alignment=AC)
    ...
    _cell(ws, 91, 4, 'once', B, alignment=AC); _cell(ws, 91, 5, 'every 6h', B, alignment=AC)
    ...
    ws.merge_cells('H91:K91'); _cell(ws, 91, 8, f'Time: {_shift_start(shift)}', B, alignment=ALT)
    ...
    _cell(ws, 93, 4, 'once', B, alignment=AC); _cell(ws, 93, 5, 'per shift', B, alignment=AC)
    ...
    _cell(ws, 94, 4, 'once', B, alignment=AC); _cell(ws, 94, 5, 'per shift', B, alignment=AC)
    ...
    _cell(ws, 95, 4, 'once', B, alignment=AC); _cell(ws, 95, 5, 'Per shift', B, alignment=AC)
    ...
    _cell(ws, 96, 4, '5 pieces', B, alignment=AC); _cell(ws, 96, 5, 'per shift', B, alignment=AC)
    ...
    _cell(ws, 97, 3, 'Module should be free from Tape,Dust,Dirt,EVA/Backsheet residue,Corner Burrs,Glue residue on (glass,backsheet,JB,Wire etc.)', B, alignment=AC)
    ws.merge_cells('D97:D101'); _cell(ws, 97, 4, '5 pieces', B, alignment=AC)
    ws.merge_cells('E97:E101'); _cell(ws, 97, 5, 'per shift', B, alignment=AC)
    ws.merge_cells('F97:G101'); _cell(ws, 97, 6, 'Post Lam Visual Criteria', Font(name='Calibri', size=18), alignment=AC)
    ...
    _cell(ws, 102, 4, 'once', B, alignment=AC); _cell(ws, 102, 5, 'per shift', B, alignment=AC)
    ...
    _cell(ws, 103, 4, 'once', B, alignment=AC); _cell(ws, 103, 5, 'per shift', B, alignment=AC)
    ...
    _cell(ws, 104, 4, 'once', B, alignment=AC); _cell(ws, 104, 5, '12h', B, alignment=AC)
    ...
    _cell(ws, 105, 4, 'once', B, alignment=AC); _cell(ws, 105, 5, 'every 6h', B, alignment=AC)
    ...
    _cell(ws, 106, 4, 'once', B, alignment=AC); _cell(ws, 106, 5, 'Two weeks', B, alignment=AC)
    ...
    _cell(ws, 113, 4, 'once', B, alignment=AC); _cell(ws, 113, 5, 'Shift', B, alignment=AC)
    ...
    ws.merge_cells('E114:E118'); _cell(ws, 114, 5, 'per shift', B, alignment=AC)
    ws.merge_cells('F114:G116'); _cell(ws, 114, 6, 'Post EL Inspection Criteria', F14, alignment=AV)
    ...
    ws.merge_cells('F117:G118'); _cell(ws, 117, 6, 'Post EL Visual Criteria', F14, alignment=AV)
    ...
    _cell(ws, 119, 4, 'once', B, alignment=AC); _cell(ws, 119, 5, 'per shift', B, alignment=AC)
    ...
    _cell(ws, 120, 4, 'once', B, alignment=AC); _cell(ws, 120, 5, 'per shift', B, alignment=AC)
    ...
    ws.merge_cells('D121:D125'); _cell(ws, 121, 4, '5 pieces', B, alignment=AC)
    ws.merge_cells('E121:E125'); _cell(ws, 121, 5, 'per shift', B, alignment=AC)
    ws.merge_cells('F121:G125'); _cell(ws, 121, 6, 'Post lam visual inspection criteria', F14, alignment=AV)
    ...
    ws.merge_cells('E126:E130'); _cell(ws, 126, 5, 'per shift', B, alignment=AC)
    ...
    _cell(ws, 131, 4, 'once', B, alignment=AC); _cell(ws, 131, 5, 'per shift', B, alignment=AC)
    ...
    _cell(ws, 132, 4, 'once', B, alignment=AC); _cell(ws, 132, 5, 'Per shift', B, alignment=AC)
    ...
    _cell(ws, 133, 4, 'once', B, alignment=AC); _cell(ws, 133, 5, 'Per shift', B, alignment=AC)
    ...
    _cell(ws, 134, 4, 'once', B, alignment=AC); _cell(ws, 134, 5, 'Per shift', B, alignment=AC)
    ...
    _cell(ws, 135, 4, 'once', B, alignment=AC); _cell(ws, 135, 5, 'Per shift', B, alignment=AC)
    ...
    _cell(ws, 136, 4, 'once', B, alignment=AC); _cell(ws, 136, 5, 'Per shift', B, alignment=AC)
    ws.merge_cells('F136:G137'); _cell(ws, 136, 6, 'WI For Packaging', F14, alignment=AV)
    ...
    _cell(ws, 137, 4, 'once', B, alignment=AC); _cell(ws, 137, 5, 'Per shift', B, alignment=AC)
    ...
    _cell(ws, 138, 4, 'once', B, alignment=AC); _cell(ws, 138, 5, 'Per shift', B, alignment=AC)
    ...
    _cell(ws, 139, 4, 'once', B, alignment=AC); _cell(ws, 139, 5, 'Per shift', B, alignment=AC)
