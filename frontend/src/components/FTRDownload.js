...
const FTRDownload = () => {
  ...
  const downloadFTRByQuantity = async () => {
    ...
    try {
      const response = await fetch('http://93.127.194.235:5002/api/master/download-ftr-by-quantity', {
        method: 'POST',
  ...
  const downloadFTRByRange = async () => {
    ...
    try {
      const response = await fetch('http://93.127.194.235:5002/api/master/download-ftr-by-serials', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          order_id: parseInt(selectedOrder),
